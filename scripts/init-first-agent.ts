/**
 * Init the first (or Nth) FrontLane agent for a DM channel.
 *
...
async function main(): Promise<void> {
  ...
  initGroupFilesystem(ag, {
    instructions:
      `# ${args.agentName}\n\n` +
      `You are ${args.agentName}, a FrontLane agent for ${args.displayName}. ` +
...
async function sendWelcomeViaCliSocket(
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    socket.once('error', (err) =>
      settle(
        new Error(
          `CLI socket at ${sockPath} not reachable: ${err.message}. Is the FrontLane service running?`,
        ),
      ),
    );
    socket.once('connect', () => {

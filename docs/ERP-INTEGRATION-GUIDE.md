# Backend Integration Guide — 后端开发者教程

面向负责实现后端网关的开发者。读完应该能从零搭出一个跟 AgentDesk 对接的最小可用网关，并知道怎么扩展到生产规模。

> 本文以 ERP 系统作为贯穿示例，但网关协议本身是通用的——后端可以是 ERP、CRM、工单系统或任意内部系统。把示例里的 `callErp` / `mapToErpUser` 替换成你自己的后端即可。
...
app.listen(3001, () => console.log('backend gateway on :3001'));
```
...
然后让 frontdesk 发"帮我查订单 INV-001"，看 `data/v2.db` 的 `gateway_audit`。
...
```ts
async function mapToErpUser(agentdeskUserId: string): Promise<ErpUser | null> {
  // 'feishu:ou_xxx' → 'ou_xxx'
  const [channel, handle] = agentdeskUserId.split(':');
...
agent 需要 `gateway_get_approval_status` 类的工具？建议在 `gateway_describe` 里暴露专用 operation：
...
agent 收到后通常会回复用户"已提交审批"。审批通过后由审批系统主动推后端，跟 AgentDesk 没关系。AgentDesk 不需要知道结果。

如果业务上必须 agent 知道结果：审批通过后由你们的系统通过另一个通道（飞书消息推送给原用户、或调用 AgentDesk 的某个 schedule_task webhook）告知。
...
AgentDesk 侧（每个 group 的 `container.json`）：
```json
{
  "backendGateway": {
...
请求头（前缀随 `BRAND_NAMESPACE` 派生，默认 `agentdesk`）：
```
x-agentdesk-timestamp: <ISO 8601>
x-agentdesk-nonce: <随机字符串>
x-agentdesk-signature: <hex>
```
...
const SIGNING_KEY = process.env.GATEWAY_SIGNING_KEY!; // 跟 AgentDesk 一致

function verifySignature(req: express.Request, rawBody: string): boolean {
  const ts = req.header('x-agentdesk-timestamp');
  const nonce = req.header('x-agentdesk-nonce');
  const sig = req.header('x-agentdesk-signature');
...
| 字段 | 来源 |
|---|---|
| `agentdesk_user_id` | `requester.userId` |
...
跟 AgentDesk 的 `gateway_audit` **互为参照**：
- AgentDesk 那边记的是"我发了什么调用，结果是什么 HTTP code"
...
`gateway_memory_get` / `gateway_memory_upsert` 给 agent 一个跨会话的 KV 存储。
...
实现要点：
- 按 `(agentdesk_user_id, namespace, key)` 三元组主键
...
2. **error message 别裸露 stack**：返给 AgentDesk 的 error 是 agent 看的，agent 会读出来给用户。脱敏。
3. **memory 加密**：含敏感信息的 value 用 AES-256 加密，DB 只存密文。
4. **保留期**：审计日志按合规要求保留（中国 PIPL 一般 3 年）。memory 跟员工绑定，离职后清。
5. **跨境**：如果 AgentDesk host 在境内、后端在境外（或反之），全部走境内代理 + 数据本地化。
...
AgentDesk 默认 `timeoutMs=15000`。超时 → agent 看到 `gateway_timeout`，会跟用户说"系统繁忙稍后重试"。
...
### 12.2 集成测试 (用 AgentDesk 当 client)
...
# 看 gateway_audit
sqlite3 data/v2.db "select * from gateway_audit order by created_at desc limit 5"
```

### 12.3 Chaos 测试

- 网关返 500：AgentDesk 应该让 agent 跟用户说"系统暂时不可用"
- 网关 timeout：同上
- 网关返 200 但 body 不合协议：AgentDesk 应记 audit + agent 友好降级
...
- [ ] 幂等 24h 窗口
- [ ] PII redaction 在写日志前
- [ ] 审计有索引（`(agentdesk_user_id, created_at)` / `(operation, created_at)`）
...
### 14.1 AgentDesk 报"gateway returned 401"
...
- 看 AgentDesk `gateway_audit` 的 `requester_source`
...
- [RUNBOOK.md](RUNBOOK.md) — 运维手册（AgentDesk 侧的 audit / 故障排查）

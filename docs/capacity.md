...
- ~~**公平性**~~ **已做(ADR-0065)**,但只做了一层:准入按**最少占用桶优先**,
  所以一次委派扇出不会把后来者压在整整 N 个模型回合之后。
  **仍未做**:公平的单位是桶不是工作量——10 分钟的回合与 5 秒的回合被同等对待。
  要按时间加权得先有 `container_lifetime_seconds` 的生产分布。
  另外桶是逐坑位的:长期保持 3 个会话的用户仍占 3 个坑,
  这里只保证新来者不必排在他后面。

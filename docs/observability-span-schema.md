...
3. 未来 Phase 0.5 / 1 / 2A / 2B / 3 扩展时，namespace 如何继续长而不乱。
本版本同时吸收了：
...
2. **Hierarchical extensibility**
   - MUAP 目前只 shipped 11 个 unique names，但未来 observability surface 会扩到 channel、agent、provider、MCP、ERP、DB、mock、hardware、GUI、python skill。
...
这五条 goal 共同约束一个核心判断：
> **一个好的 MUAP span name = 可扩展的层级 + 可聚合的 low-cardinality + Phoenix 可理解的语义入口。**
---
## §2. Naming Grammar
...
5. **present-tense / operation form only**
   ...
   - outcome 不在 name 里表达；成功/失败应写入 `status.code`、`status.message`、exception events、`muap.failure.category`。
...
| `<subsystem>.<entity>.<action>` | subsystem handling a stable entity with one action | `delivery.message.deliver`, `db.audit.write`, `module.erp_audit.emit` | encoding payload details in the third segment |
...
| `agent.*` | container-side agent lifecycle / turn loop | `AGENT` | runner root / child spans after context extraction | Planned Phase 0b+ | `agent.turn` | `container/agent-runner/src/` |
| `provider.*` | LLM provider request boundary | `LLM` | model invocation span | Planned Phase 0b+ | `provider.request` | `container/agent-runner/src/providers/` |
| `mcp.*` | MCP tool surface grouped by tool family | `TOOL` | tool invocation span | Planned Phase 0b+ | `mcp.core.send_message` | `container/agent-runner/src/mcp-tools/` |
| `erp.*` | ERP gateway HTTP / RPC contract boundary | `TOOL` | gateway call span | Planned Phase 2A+ | `erp.call` | `container/agent-runner/src/mcp-tools/erp-gateway.ts`, `scripts/configure-enterprise-gateway.ts` |
...
| `mock.*` | simulation / mock cluster / test doubles | `TOOL` | synthetic external-system span | Planned Phase 0.5 | `mock.erp.execute` | `scripts/`, future mock services |
| `hardware.*` | hardware-facing service boundaries | `TOOL` | external device / lab-system span | Planned Phase 2B | `hardware.robot.execute` | future hardware services behind ERP gateway |
| `gui.*` | GUI automation service boundaries | `TOOL` | desktop/UI automation span | Planned Phase 3 | `gui.chromeleon.run` | future GUI services behind ERP gateway |
| `python_skill.*` | Python skill service execution boundary | `TOOL` | external Python skill span | Planned Phase 2B+ | `python_skill.run` | future Python skill services |
**Inventory reconciliation**：本 registry 是 **20 top-level namespaces + 1 reserved family**。
**Reserved family note**：`mcp.knowledge.*` 暂时不是第 21 个顶层 namespace；它是 `mcp.*` 之下的 reserved family，直到 Phase 2A 对 knowledge tooling 做出更高阶拆分前，都必须继续留在 `mcp.*` 内。
...
| `agent.cancel` | `cancel` | explicit cancellation handling | Phase 1+ cancel broker integration |
### 4.8 `provider.*`
...
| Span | Allowed action | Meaning | Notes |
|---|---|---|---|
| `erp.call` | `call` | outbound call from MUAP tool layer into ERP gateway | typically child of `mcp.erp.execute` |
### 4.11 `identity.*`
...
| `erp_audit` | `src/modules/erp-audit/` | `emit`, `flush` | `module.erp_audit.emit`, `module.erp_audit.flush` | ERP audit pipeline |
...
| `cancel_broker` | `src/modules/cancel-broker/` (Phase 1) | `request`, `settle` | `module.cancel_broker.request`, `module.cancel_broker.settle` | future cancel contract |
...
在 MUAP 里，review gate 首先检查 `openinference.span.kind` 是否正确；OTel `SpanKind` 作为 secondary transport detail，可按实现边界补充。
...
- outcome is already represented by `status.code`, `status.message`, exception events, and `muap.failure.category`.
...
- Fine-grained SQL tracing belongs to other observability layers, not MUAP manual span naming.
**References**
- §4.12 `db.*`
- §6 Forbidden Patterns
- §9 Operational Discipline
...
## §9. Operational Discipline
### 9.1 Scope rule（S1）
本 schema governs MUAP **manual `withSpan(...)`** calls。
这句话有两个约束：
1. future host / runner code 写 manual spans 时，name 与 required attributes 必须 obey 本文件；
2. third-party auto-instrumentation spans（例如 provider SDK 自带 spans、HTTP client auto spans）可以作为 children 存在，但它们不控制 MUAP 的 naming registry。
换句话说：
- auto spans can coexist;
- auto spans cannot redefine the MUAP surface;
...
- add `muap.failure.category` for stable business-level filtering
示例：
```typescript
import { SpanStatusCode } from '@opentelemetry/api';
...
try {
  ...
  span?.setAttributes({
    'muap.failure.category': 'provider_timeout',
...
本节回答：“当 MUAP 继续长大时，schema 怎么扩，不至于失控？”
### 10.1 Phase-oriented extension map
| Phase / surface | Registered direction | Naming notes |
|---|---|---|
| Phase 0.5 mock cluster | `mock.<service>.*` | service details stay in attributes when dynamic |
| Phase 1 cancel broker | `module.cancel_broker.*`, `agent.cancel` | module family keeps top-level stable |
| Phase 2A knowledge | `mcp.knowledge.*` | remains under `mcp.*` until explicit elevation |

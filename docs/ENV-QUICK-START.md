...
| `AGENTDESK_SCOPE_TTL_DAYS`               | `0` (off)            | Per-user state-scope retention (ADR-0061). Expires ABANDONED scopes — the clock is refreshed on every access, so an active person's memory never expires, and a scope backing an active session is never removed at any age. A scope the sweeper has not seen is ADOPTED: stamped with the owner's real last-use time when a session still knows it, else with now. Adoption never deletes WITHOUT evidence; an adoption backed by evidence older than the window does expire on that tick. **Back up `data/v2-scopes/` first — `scripts/backup.sh` covers SQLite only.** Expiry ≠ erasure: a GDPR deletion request is a separate operation. |
| `AGENTDESK_SCOPE_TTL_DRY_RUN`            | `false`              | Preview mode for the above: log what WOULD be expired and write nothing at all (not even adoption stamps). Use it to choose a window on a live deployment.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |

...
- **🔒 评审副产物(安全切片)— ✅ 已落地(ADR-0019 硬化)**:审批卡的 fail-closed operator gate 现在**按设计触发**,不再依赖投递目标 id 类型的巧合。
  - **核查纠正**:原 finding 说"审批卡从不触发 gate"在 `requestApproval` 投递载荷层面成立(不带 `expectedUserId`),但 **Feishu 原生 render**(`feishu.ts` 旧 L953)会从投递目标推导——`openDM` 产 `feishu:p2p:ou_xxx` → `resolveReceiveTarget` 判 `open_id` → 主流 open_id 审批人**其实已被门控**。真缺口更窄:门控是**巧合 fail-closed**(依赖 `p2p:ou_` 格式撞上 id 类型启发式),对**非 open_id 审批人**(union_id `on_`、raw user_id、运营者配置的句柄)静默 open。
  - **实现**:新增渠道无关 `approverExpectedUserId()`(`channels/ask-question.ts`,strip namespace → operator 句柄)+ 四个审批卡投递点(`approvals/primitive.ts`、`approvals/onecli-approvals.ts`、`permissions/sender-approval.ts`、`permissions/channel-approval.ts`)把选定审批人 id 线程进 `ask_question` 载荷;Feishu render 用 `resolveAskQuestionExpectedUserId()` **优先**显式值、**回退**到既有 `receiveIdType==='open_id'` 推导(不回归既有 open_id 路径、不影响容器 `ask_user_question`)。
  - **landmine 处理**:render 只认 **open_id 形(`ou_`)** 的显式值——gate 比 `operator.open_id`,scope 到非 open_id 会**拒掉合法审批人**;非 open_id 显式值回退(open,不劣于现状)而非误 scope。
  - **测试**:`feishu.test.ts`(scoped 卡拒错/缺操作者、收匹配审批人;open_id-DM 路径仍 scope 无回归;normalize 后 id == operator open_id)+ `approvals/request-approval.test.ts`(`requestApproval` 投递载荷带 `expectedUserId`)。全套 771 绿、typecheck/lint 净。`approval_operator_identity` 策略指标路径未动。

...
| `ociRuntime`     | string                           | unset          | host    | OCI runtime for this group's containers (ADR-0058), e.g. `runsc` (gVisor) or `kata-runtime`; maps to `docker run --runtime`. Precedence: this field > host-wide `CONTAINER_OCI_RUNTIME` env > engine default (`runc`). Invalid values degrade to unset with a warning — never spawn-blocking.                                                                                                                                                       |

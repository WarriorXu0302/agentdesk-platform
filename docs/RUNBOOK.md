...
ADR-0019 的 fail-closed 守卫拒绝一次请求时,会 emit
`agentdesk_policy_check_failed_total{policy,reason}`,让你能区分守卫是**频繁触发**
(配置错)还是**从不触发**(策略写废了)。`policy` 取值:

- `command_gate` / `admin_denied` — 一条 admin 斜杠命令被命令闸拒(同时进 `enterprise_audit`
  的 `command_gate_deny`,§5)。偶发正常(有人探权限);持续高 = 权限配置或菜单引导有问题。
- `engage_pattern` / `invalid_regex` — 某 agent 的 engage 正则没编译过(详见 §3.9,另有专用
  指标 `engage_pattern_invalid_total` + 告警)。
- `approval_operator_identity` / `mismatch`|`absent` — 审批卡动作因操作者身份**未确认**
  (`absent`)或**与卡面向用户不符**(`mismatch`)被拒。这是**安全相关**:持续非 0 触发告警
  `AgentDeskApprovalIdentityRejections`,排查卡的受众配置是否写错,或是否有人在点别人的审批卡。

```promql
# 各 fail-closed 守卫的触发速率(分 policy/reason)
sum by (policy, reason) (rate(agentdesk_policy_check_failed_total[15m]))
```

处置:`mismatch`/`absent` 看 host stderr 的 `Feishu card action rejected` warn(带
`expectedUserId` / `operatorUserId`);`admin_denied` 看 `enterprise_audit`;`engage_pattern`
走 §3.9。

### 3.12 worker 路由反馈 / 持续 nack（`routing_feedback_total`）

worker 经 `report_routing_feedback` 回报"误投"(`misroute`)或"我不接这轮"(`nack`),平台
**只记录、不重投**(active reroute 在 ADR-0040 被否决——共享 inbound.db 身份污染会让被注入的
worker 重定向他人消息)。所以**持续**的 per-worker nack 速率意味着这些工作正在被丢(除非 worker
还顺手回了用户),触发 `AgentDeskWorkerNackingPersistently`(warning)。

`misroute` 是良性信号,不告警——它是 2.1 的误路由学习语料,从 `classification_log` 查。

排查持续 nack:先确认是不是 frontdesk 一直误派给某个 worker(配置/prompt 漂移),还是有人在
注入探测。两者都看这个 worker 组的反馈行:

```sql
-- 某 worker 的近期路由反馈(reported_by = 该 worker 的 agent_group_id)
SELECT occurred_at, feedback_kind, recommended_worker AS suggested_target,
       reasoning, classification_id
  FROM classification_log
 WHERE action='routing_feedback' AND agent_group_id = ?
 ORDER BY id DESC LIMIT 50;
```

`suggested_target`(复用 `recommended_worker` 列)是 worker 的**未校验**建议,只作运营者看板提示,

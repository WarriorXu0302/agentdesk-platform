...
> **目的**：本平台 host / runner 一切 OpenTelemetry tracing 工作的方法论 single source of truth。任何 Sub-agent 在改动 `src/observability/` 或新增 span 之前，**必须**先读这一篇。
> **状态**：v1.0
...
W3C traceparent 通过 `OTEL_TRACEPARENT` 容器环境变量贯通。container-runner 内部需用 OTel propagator 解析后作为 root context。container 内部新建的 span 自动成为 host span 的 child（待 runner-side tracing 落地后）。
...
const tracer = trace.getTracer('agentdesk-host'); // tracer name derives from the brand namespace: `<namespace>-host`
...
- 现有 ADR：`docs/decisions/ADR-0011-host-otel-instrumentation.md`（host runtime 边界与 exporter 选型）
...
- **ADR-0011**: Host OpenTelemetry Instrumentation（host runtime 边界、exporter 选型、`router.deliver_to_agent` 作为 session-trace root）
- **ADR-0014**: Observability Span Naming Schema（hierarchical snake_case naming + OpenInference attribute matrix）
- **ADR-0015**: Observability Coverage Gate（schema/runtime drift CI gate）

---

本文件保留在 `docs/observability-instrumentation-methodology.md`，作为 LLM-facing source of truth。

# FrontLane Agent Platform — 平台介绍

这是一份从零开始的平台导览。读完应该知道：FrontLane 是什么、解决了什么问题、各个模块怎么协作、生产部署需要注意什么、深入哪个细节去哪查。

文档分四层：

1. [README.md](../README.md) — 一页能力清单 + 快速启动
2. **本文（PLATFORM.md）** — 系统总览 + 导航
3. `docs/<topic>.md` — 专题文档（架构、DB、隔离模型、Feishu、ERP gateway 等）
...
**FrontLane 是一个面向企业的多用户 AI agent 基础设施。**
...
| 身份 | bot 账号代表所有用户 | 每个 ERP 调用归属到**真实员工**，不可被 prompt-injection 伪造 |
| 派活 | 单一 agent 全包 | frontdesk 分流 → 专项 worker，链路上身份不漂 |
| 审计 | 没有 / 靠对话日志 | 中央 `erp_audit` 表，每次调用一行（who / what / when / 结果） |
| 容器 | 不存在 | 每个 session 独立容器，cgroup 资源限制，并发上限 |
| 观测 | 无 | Prometheus `/metrics`：路由时延、容器退出分类、分类决策、ERP 调用错误 |

适合的部署形态：**1 个 host + Docker（或 Apple Container）+ 飞书 + 一个企业自己的 ERP 后端**。规模目标：1000 员工量级。
...
└──────────────────────────┬──────────────────────────────────────────────┘
    ...
    (Bun runtime,   ...             ...
     1 session)
       ...
       ├── 调 MCP 工具（send_message, classify_intent, erp_*, etc.）
       └── 写 outbound.db
```
...
users (id="<channel>:<handle>", kind, display_name)
   │
   ├─► user_roles (owner | global admin | scoped admin)
   │
...
详见 [docs/isolation-model.md](isolation-model.md)（4 种 session 隔离模式）。
...
15. 解析 anchor → `RequestIdentity` 设置（trusted session 来源 → ERP 调用归属正确员工）
...
17. 期间 LLM 可能调 MCP tool：
    ...
    - `erp_authorize` / `erp_execute` / `erp_memory_*` 调你的 ERP gateway
18. 所有 outbound 写到 outbound.db

### 步骤 5: 投递

19. `src/delivery.ts`（1s active + 60s sweep）轮询 outbound.db
20. **system kind** 走 delivery action 注册表（schedule_task / erp_audit / classify_intent / provider_error 等）
...
25. host 把分类决策、ERP 调用都落了 audit 表
...
ERP gateway 收到的请求带 `requesterSource: 'session' \| 'agent-asserted'` —— 后者是更弱信号（无可信来源），后端可对其拒绝写操作。
...
| 闲置容器持续吃内存 | `FRONTLANE_IDLE_EXIT_MS=120000`，2 分钟空闲容器主动退（同用户连发还是用 hot 容器） |
...
状态机：active → archived → (hard-deleted)
        │            │            │
        │            │            └─ FRONTLANE_ARCHIVE_HARD_DELETE_DAYS 后
        │            └─ tar 到 data/v2-sessions-archive/，DB 行留作审计
        └─ FRONTLANE_SESSION_TTL_DAYS 天空闲后 archive
```
...
| Container ↔ ERP gateway | 可选 HMAC-SHA256：`x-frontlane-{timestamp,nonce,signature}`（共享 signingKey） |
| OneCLI gateway | 凭证不进容器，HTTP 代理动态注入，per-agent secret 隔离 |
| 入站去重 | `inbound_dedup(channel, event_id)` 24h TTL |
| 审计表 | `erp_audit`：每次 ERP 调用（成功/失败）一行 |
...
### 5.2 ERP Gateway MCP 工具

`enterpriseGateway.baseUrl` 配置后容器自动获得：
- `erp_describe` — 列后端能力 / 操作清单
- `erp_authorize` — 操作前置鉴权
- `erp_execute` — 执行业务操作
- `erp_memory_get` / `erp_memory_upsert` — durable memory（user.profile / user.preferences / approval.history）
...
`pnpm init:enterprise` 默认建：

| folder | 业务定位 |
|---|---|
| `frontlane-frontdesk` | 接待 / 分流 / 派活 |
| `frontlane-access-worker` | 身份 / 权限 / SSO 映射 |
| `frontlane-sales-worker` | 销售相关 ERP 操作 |
| `frontlane-finance-worker` | 财务 / 发票 / 审批 |
| `frontlane-approval-worker` | 通用审批流 |
| `frontlane-ops-worker` | 运维 / 通知 |

Worker 列表是配置（CLI 参数），不是硬编码业务范围。每个 worker 独立 system prompt + container.json + 资源配额。
...
FRONTLANE_IDLE_EXIT_MS=120000
FRONTLANE_SESSION_TTL_DAYS=30
```

```bash
pnpm install
pnpm init:enterprise              # 创建 frontdesk + 5 个 worker（带默认资源限制）
pnpm configure:enterprise-gateway --base-url https://your-erp-gateway
...
| `FRONTLANE_IDLE_EXIT_MS` | 0（关） | 容器空闲多久主动退 |
| `FRONTLANE_SESSION_TTL_DAYS` | 0（关） | 多少天空闲 session 归档 |
| `FRONTLANE_ARCHIVE_HARD_DELETE_DAYS` | 0（关） | 归档多少天后物理删除 |
| `FRONTLANE_PROGRESS_STATUS_CHANNELS` | feishu | 哪些通道启用 reaction 进度提示 |
...
`/metrics` 暴露的核心指标（[full list in docs/PLATFORM.md §7]）：

```
frontlane_inbound_total{channel, outcome}                # webhook / long-connection 入站
frontlane_route_seconds{phase}                            # routeInbound / wakeContainer 时延
frontlane_session_count{agent_group, status}             # gauge，sweep 采样
frontlane_session_lifecycle_total{action}                 # archive / hard_delete 计数
frontlane_container_exits_total{agent_group, outcome}    # idle / crash / killed
frontlane_wake_rejected_total{reason}                     # 并发上限背压
frontlane_provider_errors_total{provider, code}          # 容器侧 LLM 调用错误（分桶）
frontlane_classifications_total{action}                   # 分类决策分布
frontlane_classification_bypass_total{reason, surface}   # 协议绕过观测
frontlane_classification_log_failures_total{reason}      # 审计写失败
```

接 Grafana 时建议先看的 4 个 panel：

1. `frontlane_inbound_total{outcome="accepted"}` 速率 + `outcome="deduped"` 速率
2. `frontlane_route_seconds` P50 / P95 / P99
3. `frontlane_session_count{status="active"}` + `frontlane_container_exits_total`
4. `frontlane_classification_bypass_total` —— 持续非零意味着 LLM 在跳过协议
...
- 主链路（飞书 → frontdesk → worker → ERP）端到端通
...
- 审计：ERP / classification / enterprise / inbound dedup 四张表
- 安全：飞书签名 + HMAC ERP gateway + OneCLI 凭证隔离
- 观测：10+ Prometheus 指标
- 测试：host 332 + container 169 = 501 个测试

### 还没做（不影响主链路，按需扩展）

- **可观测**：没有 Grafana dashboard 模板、没有 Langfuse trace 接入、没有 alert 规则示例
- **运维**：没有部署 runbook、没有灰度 / 滚动升级方案、没有自动化压测 harness
- **业务**：worker 的具体 ERP 操作清单是后端的事，平台只提供框架
- **多机房 / 多进程**：单 host 设计，5000+ 员工或 geo-replication 时需要重构（central DB 换 PG、部分状态走 Redis）
...
- **delivery action 是约定**：容器要让 host 做点什么（schedule task、写审计、记 metric）就 emit `kind='system', action='X'` 的 outbound，host 通过 `registerDeliveryAction('X', handler)` 接住。erp_audit / classify_intent / provider_error 都是这个模式，新增功能优先沿用。
...
**FrontLane 是一个把"多用户飞书机器人"的隔离、身份、审计、容器、观测都做完的基础设施层。** 业务能力（worker 实际能调的 ERP 操作）是后端的事；平台保证：每条消息的归属、每次 ERP 调用的审计、每个容器的资源边界都是对的。

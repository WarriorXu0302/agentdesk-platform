...
| [ADR-0034](ADR-0034-host-signing-credential-proxy.md) | Host 侧网关签名凭证代理（signingKey 不进容器，per-session 不可伪造 group 代签 + 源 IP/限速/读写分离/NO_PROXY/两阶段审计；默认 OFF 零变化；fail-closed 结构性；phase2 OpenAI 流式另开）— **设计已对抗红队验证，实现待 go/no-go** | Proposed | 2026-06-13 | `security`, `gateway`, `identity-trust-chain`, `credential-isolation` |

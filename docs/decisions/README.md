...
| [ADR-0053](ADR-0053-auto-per-group-agent-isolation.md) | **可插拔**"群 → 哪个 agent"策略(autowire 扩展,用户："配置太麻烦,能不能自动一群一 group" + "可插拔")—— 把决策点做成**策略表**(`registerGroupAgentStrategy(name,fn)` / `listGroupAgentStrategies()`),用 `ENTERPRISE_AUTO_WIRE_GROUP_STRATEGY=<name>` 选(默认 `shared`)。内置 `shared`(所有群→共享前台,**原行为**)与 `per-group`(**新群** autowire 时用 `platform_id` 派生确定性 folder **resolve-or-create** 一个**克隆自前台**的 per-group agent_group,独立 workspace+记忆);老开关 `ENTERPRISE_AUTO_WIRE_GROUP_ISOLATED=true` 留作 `per-group` **向后兼容别名**。运营者**注册自己的策略**即可落地任意拓扑(org 池化/渠道分流/固定专家)**不改 core**;未知策略名**失败安全回退 `shared`**(只 warn,绝不丢消息)。**DM/p2p 永远走共享前台**。关键前提让 per-group 变简单:**CLAUDE.md 每次 spawn 现场合成 + 技能 spawn 时 symlink**,故克隆拷 **container.json+prompts/ 并镜像委派边(双向;2026-08-16 修正,原"只拷 container.json"与 ADR-0054 组合会产生永远起不来的克隆)**;幂等+防竞态(UNIQUE 冲突 re-fetch);继承前台 org(ADR-0052);策略门在 provision 前判定**不留孤儿**;顺带**白送跨群记忆隔离**。**非新安全闸绕过**(autowire 本就 opt-in 免审批,只改"接到哪")。**不多花容器**(容器数=活跃 session 数,与 agent_group 分法无关)。诚实:这是**新特性**非之前漏做(ADR-0052 是 org 访问轴,不同)。host 全绿(+8 用例)。 | Accepted | 2026-06-16 | `autowire`, `topology`, `isolation`, `enterprise`, `backward-compat`, `pluggable` |
| [ADR-0054](ADR-0054-frontdesk-enforced-dual-llm-routing.md) | Frontdesk 强制双 LLM Routing + Execution：Routing 由独立、无工具、无 continuation 的 provider/model 产出封闭决策，runner 校验并通过共享 DB gate 强制执行；`delegate` 直达唯一 worker，其他动作再调用 frontdesk Execution。两角色可分别使用 OpenAI-compatible 或 Claude；默认关闭以保留单 provider/Claude 行为。 | Accepted | 2026-07-19 | `frontdesk`, `routing`, `provider`, `llm`, `backward-compat` |

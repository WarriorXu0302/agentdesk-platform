...
| [ADR-0053](ADR-0053-auto-per-group-agent-isolation.md) | 自动"每群一个 agent"隔离模式（autowire 扩展,用户："配置太麻烦,能不能自动一群一 group"）—— 默认 autowire 把所有群接到**一个共享 frontdesk**(共享 workspace/记忆);加 `ENTERPRISE_AUTO_WIRE_GROUP_ISOLATED`(默认 false):开启后**新群** autowire 时用 `platform_id` 派生的确定性 folder **resolve-or-create** 一个**克隆自前台**的 per-group agent_group(独立 workspace+记忆),群接到它;**DM/p2p 仍走共享前台**。关键前提让方案变简单:**CLAUDE.md 每次 spawn 现场合成 + 技能 spawn 时 symlink**,故克隆**只需拷 container.json**。幂等+防竞态(UNIQUE 冲突 re-fetch);继承前台 org(ADR-0052);策略门在 provision 前判定**不留孤儿**;顺带**白送跨群记忆隔离**。**非新安全闸绕过**(autowire 本就 opt-in 免审批,只改"接到哪")。**不多花容器**(容器数=活跃 session 数,与 agent_group 分法无关);只多 per-group 工作区目录。诚实:这是**新特性**非之前漏做(ADR-0052 是 org 访问轴,不同)。host 860/860(+5)。 | Accepted | 2026-06-16 | `autowire`, `topology`, `isolation`, `enterprise`, `backward-compat` |

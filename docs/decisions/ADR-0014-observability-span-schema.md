...
- **Tags**: `observability`, `tracing`, `naming`, `phoenix`, `openinference`
- **Supersedes**: None
- **Superseded by**: None
...
Host instrumentation initially shipped 11 ad-hoc spans with inconsistent attribute keys, no documented hierarchy, and no OpenInference attribute discipline. Phoenix UI grouping fragmented because the system had business-meaningful spans, but no binding registry for how they should be named or what semantic keys they must carry.
...
- `docs/observability-instrumentation-methodology.md` v1.0 explains how the platform should instrument Phoenix/OpenInference traces, but it is methodology, not a naming registry.
- Future work will add roughly 85% more span namespaces across mock, agent, provider, MCP, gateway, hardware, GUI, and Python skill surfaces.
...
- **Option B**: Vercel-AI-style flat names such as `ai.generateText`. Short and familiar, but a poor fit for the platform's business spans across routing, delivery, gateway, DB, and module boundaries.
...
- **Positive**: Manual spans now have one binding source of truth for names, namespace ownership, attribute requirements, and migration intent. Phoenix grouping and cross-team reviews become much more consistent.
...
- **Neutral / Trade-offs**: The schema governs the platform's manual spans only. Third-party auto-instrumentation may still emit child spans with their own names, but those spans do not control the registry.

...
1. **明文 LLM key 进容器 env**。`openai` / `codex` provider 会把
   `OPENAI_API_KEY`（以及 `OPENAI_BASE_URL` 等）通过 `docker run -e` 注入容器环境
   （`src/providers/openai.ts`）。这是底层 SDK 直连 OpenAI 兼容 Responses API 的
   **硬需求**：SDK 必须能读到 key 才能发起调用，所以 key 必须在容器内可见——这一点
   **无法消除**。（对比：`claude` provider 走 OneCLI 网关代理，容器内只有
...
对 `claude` provider 可以（OneCLI 代理已经做到）。但对 `openai` / `codex`，明文 key
是 SDK 直连的必需品，从架构上消除它需要在宿主侧再立一个"网关签名/凭证代理"
（见 §6 后续工作）——那是架构级改造，不在本批范围。在那之前，**egress 管控是真正可
落地的缓解**：既然密钥拿不掉，那就锁住它能被发去哪里。

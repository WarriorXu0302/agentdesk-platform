...
- `session_mode`: `shared` (one session per channel), `per-thread` (one per thread), `agent-shared` (one per agent group across all channels), `per-user` (one per sender in the chat), `per-user-per-thread` (one per sender and thread).

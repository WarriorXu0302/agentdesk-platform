...
async function routeInboundInner(event: InboundEvent): Promise<void> {
  ...
  if (!found) {
    ...
    agentCount = maybeAutowireEnterpriseFrontdesk(mg, event) ? 1 : 0;
  } else {
    mg = found.mg;
    agentCount = found.agentCount;
  }

  if (agentCount === 0 && maybeAutowireEnterpriseFrontdesk(mg, event)) {
    agentCount = 1;
  }
  ...
  for (const agent of agents) {
    ...
    const engages = evaluateEngage(agent, messageText, isMention, mg, event.threadId, effectiveSessionMode, userId);
    const sessionIdentityOk = !isUserScopedSessionMode(effectiveSessionMode) || userId !== null;

    if (engages && !sessionIdentityOk) {
      log.warn('User-scoped session mode refused message because sender identity is missing', {
...
function resolveEffectiveSessionMode(
  ...
  if (!adapterSupportsThreads || !isGroupChat) return sessionMode;
  if (sessionMode === 'agent-shared' || sessionMode === 'per-thread' || sessionMode === 'per-user-per-thread') {
    return sessionMode;
  }
  if (sessionMode === 'per-user') return 'per-user-per-thread';
  return 'per-thread';
}
 ...
 *   'mention-sticky' — platform mention OR an active sticky session
 *                      already exists for this routing scope. The scope is
 *                      determined by the effective session mode:
 *                      (agent), (agent, mg), (agent, mg, thread),
 *                      (agent, mg, user), or (agent, mg, user, thread).
 *                      Once that scope has engaged us once, follow-ups
 *                      arrive with no mention and should still fire.
 */
function evaluateEngage(
  ...
  switch (agent.engage_mode) {
    ...
    case 'mention-sticky': {
      ...
      // Sticky follow-up: a session already exists for this resolved
      // routing scope, so a previous mention activated it.
      if (mg.is_group === 0) return false; // DMs never use mention-sticky sensibly
      switch (effectiveSessionMode) {
        case 'agent-shared':
          return findSessionByAgentGroup(agent.agent_group_id) !== undefined;
        case 'per-user':
        case 'per-user-per-thread': {
          if (!userId) return false;
          const lookupThreadId = effectiveSessionMode === 'per-user' ? null : threadId;
          return findSessionForAgentOwner(agent.agent_group_id, mg.id, userId, lookupThreadId) !== undefined;
        }
        case 'shared':
        case 'per-thread': {
          const lookupThreadId = effectiveSessionMode === 'shared' ? null : threadId;
          const existing = findSessionForAgent(agent.agent_group_id, mg.id, lookupThreadId);
    ...
    return tracer.startActiveSpan(
      ...
      async (rootSpan) => {
        try {
          ...
          // Store live span — delivery will set output.value and end it
          ...
          if (freshSession) {
            ...
            if (!woke) {
              ...
              failSessionRootSpan(freshSession.id, 'container wake failed');
            }
          }
        } catch (error) {

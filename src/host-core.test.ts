...
describe('session manager', () => {
  ...
  it('should isolate sessions per user in per-user mode', () => {
    const { session: s1 } = resolveSession('ag-1', 'mg-1', null, 'per-user', 'discord:user-1');
    const { session: s2 } = resolveSession('ag-1', 'mg-1', null, 'per-user', 'discord:user-2');
    const { session: s1Again, created } = resolveSession('ag-1', 'mg-1', null, 'per-user', 'discord:user-1');

    expect(s1.id).not.toBe(s2.id);
    expect(s1.owner_user_id).toBe('discord:user-1');
    expect(s2.owner_user_id).toBe('discord:user-2');
    expect(created).toBe(false);
    expect(s1Again.id).toBe(s1.id);
  });

  it('should isolate sessions per user and thread in per-user-per-thread mode', () => {
    const { session: s1 } = resolveSession('ag-1', 'mg-1', 'thread-1', 'per-user-per-thread', 'discord:user-1');
    const { session: s2 } = resolveSession('ag-1', 'mg-1', 'thread-2', 'per-user-per-thread', 'discord:user-1');
    const { session: s3 } = resolveSession('ag-1', 'mg-1', 'thread-1', 'per-user-per-thread', 'discord:user-2');

    expect(s1.id).not.toBe(s2.id);
    expect(s1.id).not.toBe(s3.id);
  });

  it('should reject user-scoped session resolution without an owner user id', () => {
    expect(() => resolveSession('ag-1', 'mg-1', null, 'per-user')).toThrow(
      'ownerUserId is required for session_mode=per-user',
    );
    expect(() => resolveSession('ag-1', 'mg-1', 'thread-1', 'per-user-per-thread')).toThrow(
      'ownerUserId is required for session_mode=per-user-per-thread',
    );
  });
...
describe('router', () => {
  beforeEach(() => {
    process.env.ENTERPRISE_FRONTDESK_FOLDER = 'agentdesk-frontdesk';
    ...
    process.env.ENTERPRISE_AUTO_WIRE_GROUP_SESSION_MODE = 'per-user';
  ...
  it('enterprise auto-wires a first feishu DM to the frontdesk agent', async () => {
    ...
    await routeInbound({
      channelType: 'feishu',
      platformId: 'feishu:p2p:ou-user-1',
      threadId: null,
      message: {
        id: 'msg-feishu-dm-1',
        kind: 'chat',
        content: JSON.stringify({ senderId: 'ou-user-1', sender: 'User 1', text: 'hello desk' }),
        timestamp: now(),
        isMention: true,
        isGroup: false,
      },
    ...
    expect(rows).toHaveLength(1);
    expect(JSON.parse(rows[0].content).text).toBe('hello desk');
    expect(wakeContainer).toHaveBeenCalledTimes(1);
  });

  it('enterprise autowire uses group-safe defaults for feishu mentions', async () => {
    ...
    const { maybeAutowireEnterpriseFrontdesk } = await import('./enterprise-autowire.js');
    const { getMessagingGroup, getMessagingGroupAgents } = await import('./db/messaging-groups.js');

    const mg = getMessagingGroup('mg-feishu-group')!;
    const wired = maybeAutowireEnterpriseFrontdesk(mg, {
      channelType: 'feishu',
      platformId: 'feishu:oc_group_1',
      threadId: 'thread-1',
      message: {
        id: 'msg-feishu-group-1',
        kind: 'chat',
        content: JSON.stringify({ senderId: 'ou-user-1', sender: 'User 1', text: '@bot please help' }),
        timestamp: now(),
        isMention: true,
        isGroup: true,
      },
  ...
  it('creates isolated sessions per sender when session_mode=per-user', async () => {
    ...
    const { updateMessagingGroupAgent } = await import('./db/messaging-groups.js');
    updateMessagingGroupAgent('mga-1', { session_mode: 'per-user' });
    ...
    await routeInbound({
      ...
      message: {
        id: 'msg-user-1',
        kind: 'chat',
        content: JSON.stringify({ senderId: 'user-1', sender: 'User 1', text: 'hello from one' }),
        timestamp: now(),
      },
    });
    await routeInbound({
      ...
      message: {
        id: 'msg-user-2',
        kind: 'chat',
        content: JSON.stringify({ senderId: 'user-2', sender: 'User 2', text: 'hello from two' }),
        timestamp: now(),
      },
    });
    await routeInbound({
      ...
      message: {
        id: 'msg-user-1b',
        kind: 'chat',
        content: JSON.stringify({ senderId: 'user-1', sender: 'User 1', text: 'back again' }),
        timestamp: now(),
      },
    });
    ...
    expect(sessions.map((s) => s.owner_user_id).sort()).toEqual(['discord:user-1', 'discord:user-2']);
    expect(findSession('mg-1', null)).toBeUndefined();
  });

  it('mention-sticky respects per-user session isolation', async () => {
    const { routeInbound } = await import('./router.js');
    const { wakeContainer } = await import('./container-runner.js');
    const { getSessionsByAgentGroup } = await import('./db/sessions.js');
    await import('./modules/permissions/index.js');
    ...
    (wakeContainer as unknown as ReturnType<typeof vi.fn>).mockClear();
    const { updateMessagingGroupAgent } = await import('./db/messaging-groups.js');
    updateMessagingGroupAgent('mga-1', { engage_mode: 'mention-sticky', session_mode: 'per-user' });
    ...
    await routeInbound({
      ...
      message: {
        id: 'sticky-start',
        kind: 'chat',
        content: JSON.stringify({ senderId: 'user-1', sender: 'User 1', text: '@bot hey' }),
        timestamp: now(),
        isMention: true,
      },
    });
    await routeInbound({
      ...
      threadId: null,
      message: {
        id: 'sticky-follow-user-1',
        kind: 'chat',
        content: JSON.stringify({ senderId: 'user-1', sender: 'User 1', text: 'follow-up without mention' }),
        timestamp: now(),
      },
    });
    await routeInbound({
      ...
      threadId: null,
      message: {
        id: 'sticky-follow-user-2',
        kind: 'chat',
        content: JSON.stringify({ senderId: 'user-2', sender: 'User 2', text: 'should not attach to user one' }),
        timestamp: now(),
      },
    });
  ...
  it('refuses to route per-user sessions when sender identity is missing', async () => {
    ...
    await routeInbound({
      ...
      threadId: null,
      message: {
        id: 'msg-anon-user-scope',
        kind: 'chat',
        content: JSON.stringify({ sender: 'Anonymous', text: 'hello' }),
        timestamp: now(),
      },
    });
  ...
  it('does not accumulate anonymous context into per-user sessions', async () => {
    const { routeInbound, setSenderResolver } = await import('./router.js');
    const { wakeContainer } = await import('./container-runner.js');
    const { getSessionsByAgentGroup } = await import('./db/sessions.js');
    const { updateMessagingGroupAgent } = await import('./db/messaging-groups.js');
    ...
    await routeInbound({
      ...
      threadId: null,
      message: {
        id: 'msg-anon-accumulate',
        kind: 'chat',
        content: JSON.stringify({ sender: 'Anonymous', text: 'silent context' }),
        timestamp: now(),
      },
    });

...
describe('sessions', () => {
  ...
  it('should allow multiple user-scoped sessions in one messaging group', () => {
    createSession({ ...sess(), id: 'sess-u1', owner_user_id: 'discord:user-1' });
    createSession({ ...sess(), id: 'sess-u2', owner_user_id: 'discord:user-2' });
    expect(getSessionsByAgentGroup('ag-1')).toHaveLength(2);
    expect(findSession('mg-1', null)).toBeUndefined();
  });

...
export function findSession(messagingGroupId: string, threadId: string | null): Session | undefined {
  if (threadId) {
    return getDb()
      .prepare(
        'SELECT * FROM sessions WHERE messaging_group_id = ? AND thread_id = ? AND owner_user_id IS NULL AND status = ?',
      )
      .get(messagingGroupId, threadId, 'active') as Session | undefined;
  }
  return getDb()
    .prepare(
      'SELECT * FROM sessions WHERE messaging_group_id = ? AND thread_id IS NULL AND owner_user_id IS NULL AND status = ?',
    )
...
export function findSessionForAgent(
  ...
  if (threadId) {
    return getDb()
      .prepare(
        "SELECT * FROM sessions WHERE agent_group_id = ? AND messaging_group_id = ? AND thread_id = ? AND owner_user_id IS NULL AND status = 'active'",
      )
      .get(agentGroupId, messagingGroupId, threadId) as Session | undefined;
  }
  return getDb()
    .prepare(
      "SELECT * FROM sessions WHERE agent_group_id = ? AND messaging_group_id = ? AND thread_id IS NULL AND owner_user_id IS NULL AND status = 'active'",
    )
    .get(agentGroupId, messagingGroupId) as Session | undefined;
}

/**
 * Session lookup scoped to a specific agent group, messaging group, and user.
 *
 * Used by enterprise-style user-isolated session modes where multiple users
 * share one chat surface but must not share context.
 */
export function findSessionForAgentOwner(
  agentGroupId: string,
  messagingGroupId: string,
  ownerUserId: string,
  threadId: string | null,
  ...
  if (threadId) {
    return getDb()
      .prepare(
        "SELECT * FROM sessions WHERE agent_group_id = ? AND messaging_group_id = ? AND owner_user_id = ? AND thread_id = ? AND status = 'active'",
      )
      .get(agentGroupId, messagingGroupId, ownerUserId, threadId) as Session | undefined;
  }
  return getDb()
    .prepare(
      "SELECT * FROM sessions WHERE agent_group_id = ? AND messaging_group_id = ? AND owner_user_id = ? AND thread_id IS NULL AND status = 'active'",
    )
    .get(agentGroupId, messagingGroupId, ownerUserId) as Session | undefined;
}

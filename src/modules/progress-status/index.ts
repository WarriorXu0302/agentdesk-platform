...
function enabledFor(channelType: string): boolean {
  return parseEnabledChannels(process.env.NANOCLAW_PROGRESS_STATUS_CHANNELS).has(channelType.toLowerCase());
}

function resolveReactionEmoji(channelType: string): string | null {
  switch (channelType.toLowerCase()) {
    case 'feishu':
      return process.env.NANOCLAW_PROGRESS_STATUS_FEISHU_EMOJI?.trim() || DEFAULT_FEISHU_PROGRESS_EMOJI;
    default:
      return null;
  }
}

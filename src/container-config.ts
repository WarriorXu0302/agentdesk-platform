 ...
 * `provider`) — plus, now, its Routing (`llm`) block. Falling back to empty there
 * silently DROPPED those — egress lockdown off, cgroup limits gone, every skill
 * re-enabled, Routing disabled — and the very next `ensureRuntimeFields`
 * write-back persisted the empty config, destroying the evidence. So corruption
 * now fails CLOSED: the original is copied aside and the error is rethrown,
 * refusing to start the group.
 */

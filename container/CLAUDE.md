...
The file `CLAUDE.local.md` in your workspace is your per-group memory. Record things there that you'll want to remember in future sessions — orientation notes, project context, recurring facts. Keep entries short and structured. (What belongs here vs. in gateway memory depends on your memory mode — see Memory below.)
...
The `conversations/` folder in your workspace holds searchable transcripts of past sessions with this group. Use it to recall prior context when a request references something that happened before. For structured long-lived data in workspace mode, prefer dedicated files (`customers.md`, `preferences.md`, etc.), splitting any file over ~500 lines into a folder with an index; in gateway mode such durable records belong behind the gateway instead (see Memory).

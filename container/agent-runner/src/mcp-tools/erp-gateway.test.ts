...
describe('erp gateway mcp tools', () => {
  ...
  it('uses the poll-loop-published identity and ignores agent-asserted values', async () => {
    ...
    const result = await handleErpDescribe(configuredRuntime({ baseUrl: 'https://erp-gateway.example' }), {
  ...
  it('propagates origin_user_id from an a2a-delegated worker session', async () => {
    ...
    const result = await handleErpAuthorize(
      { ...runtime, agentGroupId: 'ag-worker', enterpriseGateway: { baseUrl: 'https://erp-gateway.example' } },
  ...
  it('loads durable memory from /memory/get using the trusted userId as default subject', async () => {
    ...
    await handleErpMemoryGet(configuredRuntime({ baseUrl: 'https://erp-gateway.example' }), {
  ...
  it('writes durable memory to /memory/upsert', async () => {
    ...
    await handleErpMemoryUpsert(configuredRuntime({ baseUrl: 'https://erp-gateway.example' }), {
  ...
  it('signs outbound requests with HMAC-SHA256 when signingKey is configured', async () => {
    ...
    const timestamp = capturedHeaders['x-frontlane-timestamp'];
    const nonce = capturedHeaders['x-frontlane-nonce'];
    const signature = capturedHeaders['x-frontlane-signature'];
  ...
  it('skips signing headers when no signingKey is configured', async () => {
    ...
    await handleErpDescribe(configuredRuntime({ baseUrl: 'https://erp-gateway.example' }), {});
    expect(capturedHeaders['x-frontlane-signature']).toBeUndefined();
  });

  it('emits an erp_audit system message with a path-scoped input_hash', async () => {
    setRequestIdentity(sessionIdentity());
    ...
    const auditPayloads = rows
      ...
      .filter((p): p is Record<string, unknown> => !!p && p.action === 'erp_audit');

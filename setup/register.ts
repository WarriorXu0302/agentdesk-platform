...
function isUserScopedSessionMode(mode: string): mode is 'per-user' | 'per-user-per-thread' {
...
export async function run(args: string[]): Promise<void> {
  ...
  if (!existing) {
    ...
    createMessagingGroupAgent({
      ...
      session_mode: parsed.sessionMode as 'shared' | 'per-thread' | 'agent-shared' | 'per-user' | 'per-user-per-thread',
      priority: 0,
      created_at: new Date().toISOString(),
    });
  ...
  if (newlyWired) {
    if (isUserScopedSessionMode(parsed.sessionMode)) {
      log.info('Skipping onboarding message for user-scoped session mode without sender context', {
      ...
      const { session } = resolveSession(
        ...
        messagingGroup.id,
        null,
        parsed.sessionMode as 'shared' | 'per-thread' | 'agent-shared' | 'per-user' | 'per-user-per-thread',
      );
